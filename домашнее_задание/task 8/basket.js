...
const workCart = document.querySelector('.cartIconWrap');
workCart.addEventListener('click', e => {
    document.querySelector('.basket').classList.toggle('hidden');
});

const basket = {};

const pressBtn = document.querySelector('.featuredItems');
pressBtn.addEventListener('click', e => {
    ...
    /* !!!!!
    изначально кружка у корзины нет, так правильно,
    как только добавили товар в корзину,
    появляется кружок.
    !!!!! */
    if (!basketCounterEl.classList.contains('active')) {
        basketCounterEl.classList.add('active');
    }
